...
**完了時:** AC ドラフト生成後、以下の 2 つの Agent を **1 メッセージ内で同時起動** して独立検証する:

```
Agent(subagent_type="aidd-architect", prompt="""
  以下の Epic AC セットを設計・実装可能性の観点で検証してください。

  Epic 仕様書: [docs/requirements/ES-NNN-slug.md のパス]

  確認観点:
  - 実装に必要な設計判断が AC に含まれているか（曖昧な仕様はないか）
  - AC 間に矛盾・依存関係の問題がないか
  - 既存アーキテクチャ・ADR と整合しているか
  - Task に分解できない粒度になっていないか

  問題があれば具体的に指摘してください。問題なければ「設計観点: 問題なし」と返してください。
""")

Agent(subagent_type="aidd-analyst", prompt="""
  以下の Epic AC セットを業務要件カバレッジの観点で検証してください。

  Epic 仕様書: [docs/requirements/ES-NNN-slug.md のパス]
  Phase 定義書: [docs/requirements/PD-NNN-slug.md のパス]

  確認観点:
  - Phase 定義書の機能意図を全てカバーしているか
  - ユーザーストーリーの「so that」（目的）が AC に反映されているか
  - エッジケース・エラーケースの AC が不足していないか
  - ステークホルダーの視点で見落としはないか

  問題があれば具体的に指摘してください。問題なければ「業務観点: 問題なし」と返してください。
""")
```

両 Agent の指摘を集約し、`references/briefing-spec.md` に従ってブリーフィングを実行し、承認を得る。
...
| 専門家レビュー結果 | aidd-architect / aidd-analyst の指摘と対応方針 |
