...
| ADR 参照 | <!-- 該当なし --> |
| ゲート状態 | G3: 通過（Step 2-3 スキップ: テスト専用 Epic のためドメインモデル/DB/API 設計なし） |
...
| ドメインモデル詳細 | 該当なし（テスト専用 Epic） | スキップ（G3 承認済み） |
| 集約モデル詳細 | 該当なし | スキップ（G3 承認済み） |
| DB スキーマ骨格 | 該当なし | スキップ（G3 承認済み） |
| API spec 骨格 | 該当なし | スキップ（G3 承認済み） |
