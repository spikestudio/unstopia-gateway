...
- S13: As a **開発者**, I want to `packages/jimmy` のユニットテストを拡充し branch カバレッジを段階的に引き上げたい, so that E3 CI 閾値（最終 60%）を達成でき、変更の安全性を定量的に確認できる.（ユーザー要求: テストカバレッジを増やす）
- S14: As a **開発者**, I want to テスタビリティを妨げるコードをリファクタリングしたい, so that テストが書きやすくなりカバレッジ拡充の効率が高まる.（ユーザー要求: 必要ならリファクタリングも辞さない）

> **注記:**
> - **承認済みストーリー**: S1〜S10 + S12〜S14 の計 13 件（S11 は削除）
...
| E2.5: テストカバレッジ拡充 | 支援 | ユニットテスト追加・テスタビリティリファクタリング | 標準 | ユニットテスト中心（カバレッジ率達成確認） |
...
| E5: ドキュメント整備 | 汎用 | ドキュメント整備。ドメイン固有性なし | 標準 | 不要 |
...
```mermaid
graph LR
    E2[E2: テストカバレッジ基盤] --> E25[E2.5: テストカバレッジ拡充]
    E25 --> E1[E1: Biome 警告ゼロ化]
    E1 --> E3[E3: CI 品質ゲート]
    E25 --> E3
...
**実施順序（更新）:**
1. **E2 完了済み**: カバレッジ計測基盤（@vitest/coverage-v8）設定完了。現状値 branch 9.04% 確定
2. **E2.5 次**: テストを実際に書いてカバレッジを引き上げる。テスタビリティ改善リファクタリングも含む
3. **E1 次**: テスト基盤が整った状態で Biome 警告を修正する。テストにより修正後のリグレッションを検出できる
4. **E3・E4 最後**: E1・E2.5 が完了した後に CI ゲートと lefthook を整備する
