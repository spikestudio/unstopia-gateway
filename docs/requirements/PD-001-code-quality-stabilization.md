...
```mermaid
graph LR
    E2[E2: テストカバレッジ基盤] --> E1[E1: Biome 警告ゼロ化]
    E1 --> E3[E3: CI 品質ゲート]
    E2 --> E3
...
**実施順序（確定）:**
1. **E2 先行**: カバレッジ計測で現状の数値を把握し、テスト拡充を実施する。目標値（60% or 50%）はこの段階で確定する
2. **E1 次**: テスト基盤が整った状態で Biome 警告を修正する。テストにより修正後のリグレッションを検出できる
3. **E3・E4 最後**: E1・E2 が完了した後に CI ゲートと lefthook を整備する
