...
> **スコープ注記:** S14（テスタビリティ改善リファクタリング）は今回スコープ外。リファクタリングを伴わずに純粋関数・ビジネスロジック優先でテストを追加し、branch coverage 18% を目標とする（達成値: 18.00%）。
...
現状 branch カバレッジ 15.37%（ES-002〜TASK-012 完了時点）の `packages/jimmy` に対してユニットテストを拡充し、**branch coverage 18% 以上**を目標とする（達成値: 18.00%、862/4788 branches）。
...
| カバレッジ閾値 | branch 18%（本 Epic 目標・達成値 18.00%。45% は ES-005、60% は E3 以降で段階引き上げ） | 閾値未達の場合は追加テストを実装する |
...
| カバレッジ閾値未達 | branch カバレッジが 18% 未満 | 追加テストを実装して閾値を達成する | 18% 達成困難な場合は純粋関数モジュールの網羅度を見直す |
...
|------|------|
| 対象ユーザー/ペルソナ | 開発者（自分） |
| デリバリーする価値 | `pnpm test --coverage` で branch カバレッジ 18% 以上が確認でき（達成: 18.00%）、E3 CI 閾値設定（段階的引き上げ）の前提が整う。純粋関数・ビジネスロジック中心のモジュールにテストが追加され、変更の安全性が高まる。ES-004/005（S14）の土台となる「DI 後も有効なテスト」が揃う。 |
| デモシナリオ | 1. `packages/jimmy` で `pnpm test --coverage` を実行する → 2. branch カバレッジが 18% 以上の値を示す（達成: 18.00%） → 3. `coverage/index.html` を開いて主要モジュール（`cron/`・`gateway/`・`connectors/`・`engines/`・`shared/`）がカバーされていることを確認する |

## E2E 検証計画

| 項目 | 内容 |
|------|------|
| 検証シナリオ | AC-E003-01〜05: `packages/jimmy` で `pnpm test --coverage` を実行し全テスト PASS + branch カバレッジ 18% 以上を確認（自動検証・達成: 18.00%）。`pnpm build` が PASS することを確認（手動デモ）。 |
...
| 2 | 30% → 目標値の修正: branch 18% に下方修正（2026-04-24 決定・達成値 18.00%）。理由: 大規模システムファイル（gateway/api.ts 2590行等）が 0% で分母を押し上げており、リファクタリングなしでの 30% 達成は困難。AC-E003-05（リファクタリング禁止）と整合させるため修正。ES-004/005（S14）完了後に 45% を目指す。 | 確定 | — |
