...
name: aidd-review
description: >
  コンテキストを自動検出し最適なレビュー種別を選択するオーケストレーター。
  epic-spec（仕様書）/ task-spec（Task定義）/ code（コード）/ epic（Epic総合）/ phase（Phase完了）
  の5種別を提供。観点ごとのサブエージェント並列実行・critical/non-critical全件1件ずつ合意。
