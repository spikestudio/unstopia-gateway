...
- **`context: fork` スキルの出力を要約・言い換えしてはならない（#1100）。** fork スキルがサブエージェントとして実行された場合、スキルが生成した確認メッセージ・ブリーフィング・承認要求をそのままユーザーに提示すること。「〇〇が提示されています。よいですか？」のような要約・言い換えは禁止。原文を忠実にリレーすること
