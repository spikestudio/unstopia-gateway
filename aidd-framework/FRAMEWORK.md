...
|------|-------------|---------------------|-----------|
| Phase | Issue + Milestone | Phase 定義書へのリンク + 概要 | Phase 定義書（Markdown） |
| Epic | sub-issue + PR | Epic 仕様書へのリンク + 概要 | Epic 仕様書（Markdown） |
...
- Phase Issue は全 Epic Issue が完了し、G6 判定後に手動クローズする

**ラベル:**

| ラベル | 用途 |
|--------|------|
| `phase` | Phase Issue の識別 |
| `task` | Task Issue の識別 |
| `epic` | Epic Issue の識別 |
...
| 一括確認 | `/aidd-status`（Phase Issue → Epic Issue → Task Issue のツリーと Milestone を組み合わせて表示） |

**Sub-issues による階層管理:** `/aidd-new-epic` が Epic Issue を Phase Issue の子として作成し、`/aidd-decompose-epic` が Task Issue を Epic Issue の子として作成する。PR 作成時は `Epic: #NNN` で親 Epic を示し、Sub-issues から `Closes #NNN` を自動生成して全 Task Issue を確実にクローズする。
