...
AI は常に計画（成果物・設計・AC・Task）を提示してから実行する。人間は計画を承認するだけでよい。AI が実行するすべてのアクティビティにこのループが適用される。

### 発散フェーズと収束フェーズ（ADR-015）

Inception 道具箱（G1後・G2前）は**発散フェーズ**、`/aidd-new-epic` 以降は**収束フェーズ**として明確に分離する。

| フェーズ | 目的 | 担当スキル | 成果物の性質 |
|---------|------|----------|------------|
| **発散フェーズ** | アイデア・ストーリー候補・制約・対立点を積極的に増やし拡げる | `/aidd-mob`・`/aidd-inception`（mock/api-spec/cli） | 叩き台（確定ではない） |
| **収束フェーズ** | 発散成果物を重複排除・精緻化・意思決定して実装可能な AC に収束させる | `/aidd-new-epic`・`/aidd-decompose-epic`・`/aidd-impl` | 確定成果物 |

- 発散フェーズは**任意**。スキップしても収束フェーズは成立する
- 発散フェーズの成果物は「確定した設計」ではなく「叩き台」。収束フェーズで上書き・精緻化される
- 発散フェーズで多く出すほど、収束フェーズの品質が上がる

### Phase と Epic の責務分担

| 単位 | 責務 | 内容の粒度 |
|------|------|---------|
| **Phase** | 「このマイルストーンで何を作るか」 | 機能意図の一覧（薄い・30分以内） |
| **Epic** | 「その機能をどう定義するか」 | ストーリー + AC（重い・詳細） |

Phase は機能意図を列挙し Epic にマッピングするだけ。ストーリー定義・AC 設計・実装はすべて Epic で行う。
...
> **AI-DLC との対応:** Task は AWS AI-DLC の「Bolt」（時間〜日単位の短サイクル実装単位）に相当する。Task = 「AI が1コミットで完結できる実装単位」という定義は、Bolt の「時間〜日単位で AI が実行できる作業」と等価である。（参照: `docs/research/ai-dlc-concepts.md`）
...
> **優先度 1〜6 の全ての問題を原則その場で修正する。** 「コストが高いから見送り」「現在のデータ量では問題ない」「Phase 1 では不要」「ブロッカーではない」「次の Task で対応可能」「動いているから OK」「規約に書いてないから OK」「PoC スケールでは許容範囲」「MVP だから不要」「まだプロトタイプだから」「今は小規模だから問題ない」は免除理由にならない。
>
> **プロジェクトのフェーズ・規模・成熟度（PoC・MVP・プロトタイプ・β版）を品質基準・規約遵守の免除理由にしてはならない。** 開発段階に関わらず、優先度 1〜5 の問題は無条件で対応し、優先度 6 はスコープ外の場合も別コミット・別 PR で対応する。
...
**プロセス × スキル対応（AI-DLC フェーズ対応）:**

| 位置付け | スキル | ゲート |
|---------|--------|--------|
| **Project Init**（一回限り） | `/aidd-setup` + `/aidd-skeleton` | G0 |
| **Release Planning**（Phase ごと） | `/aidd-new-phase`（機能意図列挙・Epic マッピング） | G1 |
| **Inception 道具箱**（G1後・G2前・任意） | `/aidd-mob`（ペルソナ Mob Elaboration）<br>`/aidd-inception`（画面モック / API 仕様 / CLI 仕様の対話的生成） | — |
| **Inception**（Epic ごと） | `/aidd-new-epic`（ストーリー詳細化・AC 導出） | G2 |
| | `/aidd-new-epic`（設計）<br>+ 画面系のみ: `/aidd-screen-plan` → `/aidd-screen-spec` → `/aidd-screen-ui` | G3 |
| **Construction**（Epic ごと） | `/aidd-decompose-epic` → `/aidd-impl` × Task 数 | G4 |
| **Operations**（Epic ごと） | `/aidd-review epic` | G5 |
| **Release**（Phase ごと・G1 の対） | 画面系のみ: `/aidd-screen-catalog` | — |
| | `/aidd-review phase` + `/aidd-phase-closing` | G6 |
...
| **レビューモード** | 成果物の全問題を検出する。検出漏れは AI の不具合 | **全問題検出を保証する。** 規約・AC・設計整合性を機械的に検証。検出漏れがあれば AI の改善課題とする | `/aidd-review epic`, `/aidd-review phase` |
...
| スキル | レイヤー | 責務 |
|--------|---------|------|
| `/aidd-review epic` | Epic 検証（G5） | Epic 単位の AC 準拠・設計整合性・コード品質を 3 観点で検証 |
| `/aidd-review phase` | Phase 検証（G6） | Phase 完了検証（成功基準・マスタドキュメント最新化） |
| `/aidd-phase-closing` | Phase 完了処理（G6） | G6 通過記録・Milestone クローズ |
...
| G5 | local / CI | コード品質、テスト通過、Epic 総合レビュー（/aidd-review epic） |
...
/aidd-new-phase → G1 マイルストーン確認
  → /aidd-mob（任意）/ /aidd-inception（任意）  ← Inception 道具箱
  → /aidd-new-epic → G2 + G3 マイルストーン確認（AC 定義 + 詳細設計を統合）
    → /aidd-screen-plan → /aidd-screen-spec → /aidd-screen-ui  ← 画面系のみ
    → /aidd-decompose-epic → G4 マイルストーン確認
      → /aidd-impl（Task ごとにコミット）→ /aidd-review epic → G5
  → /aidd-review phase → /aidd-phase-closing → G6 マイルストーン確認
```
...
| スキル | 用途 |
|--------|------|
| `/aidd-mob` | G1後・G2前: 複数ペルソナで Mob Elaboration を擬似実行し高品質なストーリードラフトを生成 |
| `/aidd-inception` | G1後・G2前: プロジェクト特性に応じてサブスキルを選択し具体的成果物を対話的に生成 |
| `/aidd-inception-mock` | G1後・G2前: 機能意図から画面モック + ストーリードラフトを生成（UI 系） |
| `/aidd-inception-api-spec` | G1後・G2前: 機能意図から API 仕様 + ストーリードラフトを生成（API 系） |
| `/aidd-inception-cli` | G1後・G2前: 機能意図から CLI コマンド仕様 + ストーリードラフトを生成（CLI 系） |
...
| `/aidd-review epic` | Epic 単位の統合レビュー（AC 検証・完了形式チェック・gate 制御） |
| `/aidd-review phase` | Phase 完了検証（成功基準・マスタドキュメント最新化・G6 準備） |
| `/aidd-phase-closing` | Phase 完了クロージング（G6 通過記録・Milestone クローズ） |
...
| `/aidd-review code` | 実装中の任意タイミングでコードレビューを単体実行（`--comment` で PR コメント投稿） |
