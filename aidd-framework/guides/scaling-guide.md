...
3. レビューは AI 一次判定（`/aidd-review epic`）で MUST FIX を先に解消し、人間は設計判断のみに集中
...
/aidd-review epic 実行
  ...
  → MUST FIX が 1 件以上: AI または人間が修正 → 再度 /aidd-review epic
```
...
- **AI レビュー（`/aidd-review epic`）を G5 の前提条件にする。** MUST FIX 0 件を人間レビューのエントリ条件とすることで、人間のレビュー負荷を大幅に削減できる
...
**対策:**
1. AI 一次判定（`/aidd-review epic`）の MUST FIX 0 件を前提条件とし、人間は設計判断のみに集中する
2. 並行度を下げる（推奨上限を参照）
